...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `dishes` SET name = ?, category = ?, price = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `dishes` SET image = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <?php
      ...
      $show_products = $conn->prepare("SELECT * FROM `dishes` WHERE id = ?");
